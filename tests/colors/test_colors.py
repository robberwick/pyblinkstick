...
from blinkstick.colors import remap, remap_color, remap_color_reverse, remap_rgb_value, remap_rgb_value_reverse, Color, \
    name_to_hex, normalize_hex, hex_to_rgb, name_to_rgb
...
def test_color_from_name_invalid_color():
    with pytest.raises(ValueError, match="'invalidcolor' is not defined as a named color."):
...
def test_normalize_hex_missing_hash():
    with pytest.raises(ValueError, match="'0099cc' is not a valid hexadecimal color value."):
...
def test_hex_to_rgb_invalid_hex():
    with pytest.raises(ValueError, match="'#xyz' is not a valid hexadecimal color value."):
        hex_to_rgb('#xyz')